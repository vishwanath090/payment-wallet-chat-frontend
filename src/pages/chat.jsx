// src/components/Chat.jsx - IMPROVED WITH WHATSAPP FEATURES (FIXED WS URL + TOKEN)
...
const Chat = () => {
  ...
  // ✅ FIXED WebSocket connection - uses env + token
  const connectWebSocket = useCallback(() => {
    if (!currentUser?.email || !authToken) {
      console.log("❌ No user or token for WebSocket");
    ...
    try {
      // ✅ Decide base WS URL: prefer env, fallback to localhost for dev
      let baseWsUrl = WS_BASE_URL;
      if (!baseWsUrl) {
        const isHttps = window.location.protocol === "https:";
        const host = window.location.host;
        if (isHttps) {
          baseWsUrl = `wss://${host}/api/v1/chat/ws`;
        } else {
          baseWsUrl = `ws://localhost:8000/api/v1/chat/ws`;
        }
      }

      // ✅ Append user identifier and JWT token as query param
      const wsUrl = `${baseWsUrl}/${encodeURIComponent(
        currentUser.email
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === "connection") {
            ...
            console.log(
              `📨 Message ${data.message_id}: ${data.status} for user: ${currentUser.email}`
            );
          ...
          // Handle actual chat messages
            ...
            const isForCurrentChat =
              selectedUser &&
              ((data.sender_email === selectedUser.email &&
                data.receiver_email === currentUser.email) ||
                (data.sender_email === currentUser.email &&
                  data.receiver_email === selectedUser.email));
          ...
          console.error(
            "❌ Error parsing WebSocket message for user:",
            currentUser.email,
            error
          );
        }
      ...
      console.error(
        "❌ Failed to create WebSocket for user:",
        currentUser?.email,
        error
      );
      setConnectionStatus("error");
    }
  }, [currentUser?.email, currentUser?.id, authToken, selectedUser?.email]);
  ...
  const getConnectionStatus = () => {
    switch (connectionStatus) {
      ...
      case "reconnecting":
        return { text: "Reconnecting...", color: "#F59E0B", icon: "🟡" };
      case "disconnected":
            ...
            enhancedContacts.map((user) => (
              <div
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    selectedUser?.id === user.id ? "#4B5563" : "transparent",
                  borderLeft:
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div
                  style={{
                    fontSize: "14px",
                    color: onlineUsers.has(selectedUser.email)
                      ? "#25D366"
                      : "#9CA3AF",
                    display: "flex",
                    alignItems: "center",
                    ...
                    >
                      <div
                        style={{
                          ...
                          padding: "8px 12px",
                          borderRadius: "8px",
                          borderTopRightRadius: isMyMessage(message)
                            ? "0px"
                            : "8px",
                          borderTopLeftRadius: isMyMessage(message)
                            ? "8px"
                            : "0px",
                          position: "relative",
